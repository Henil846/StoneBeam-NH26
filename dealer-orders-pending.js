...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showOrderDetails(orderId) {
        ...
        document.getElementById('modal-priority-badge').textContent = order.priority.toUpperCase();
        document.getElementById('modal-priority-badge').className = `order-priority ${order.priority}`;
        ...
        document.getElementById('modal-status').className = `status-badge ${order.status}`;
        document.getElementById('modal-requested-date').textContent = formatDate(order.requestedAt);
        ...
        timeline.innerHTML = order.timeline.map(item => `
            <div class="timeline-item ${item.status}">
                ...
                <div class="timeline-status ${item.status}">
                    <i class="fa-solid fa-${item.status === 'completed' ? 'check-circle' : 'clock'}"></i>
                </div>
            </div>
        `).join('');

        // Set up modal action buttons
        document.getElementById('contact-client-btn').onclick = () => {
            showContactOptions(order.client.email, order.client.phone);
        };

        document.getElementById('schedule-visit-btn').onclick = () => {
            scheduleVisit(order.id);
        };

        document.getElementById('submit-quote-btn').onclick = () => {
            closeModals();
            showQuickQuoteModal(order.id);
        };
