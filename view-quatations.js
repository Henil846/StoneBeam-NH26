...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadQuotations() {
        ...
        const stored = localStorage.getItem('quotationRequests');
        if (stored) {
            allQuotations = JSON.parse(stored);
        }
        
        // Add sample quotes to existing requests for demo
        allQuotations.forEach(quotation => {
            if (!quotation.quotes) {
                quotation.quotes = generateSampleQuotes(quotation);
                quotation.status = quotation.quotes.length > 0 ? 'quoted' : 'pending';
            }
        });
    ...
    function createQuoteItem(quote) {
        ...
        const rating = '★'.repeat(Math.floor(quote.contractorRating)) + 
                      (quote.contractorRating % 1 >= 0.5 ? '☆' : '') + 
                      '☆'.repeat(5 - Math.ceil(quote.contractorRating));
            ...
            <div class="quote-item">
                <div class="quote-header">
                    <div>
                        ...
                        <div class="rating" title="Rating: ${quote.contractorRating}/5">
                            ${rating} (${quote.contractorRating})
                        </div>
                    </div>
    ...
    window.acceptQuote = function(quoteId) {
        showNotification('Quote accepted! You will be redirected to the contract page.', 'success');
        // In a real app, this would update the database and redirect
        setTimeout(() => {
            // window.location.href = `contract.html?quote=${quoteId}`;
        }, 2000);
    };
